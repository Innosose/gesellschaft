...
export const MS = {
  ...
  input: (): React.CSSProperties => ({
    ...
    background: rgba(T.fg, 0.05), color: rgba(T.fg, 0.9), fontSize: 15, outline: 'none',
  ...
  group: (): React.CSSProperties => ({
    background: rgba(T.fg, 0.04), borderRadius: 12, overflow: 'hidden' as const,
  }),
  /** Group row item — iOS list row */
  groupRow: (last?: boolean): React.CSSProperties => ({
    display: 'flex', alignItems: 'center', gap: 12, padding: '0 16px',
    minHeight: 48, borderBottom: last ? 'none' : `1px solid ${rgba(T.fg, 0.06)}`,
  ...
  label: (): React.CSSProperties => ({ fontSize: 13, color: rgba(T.gold, 0.5), marginBottom: 1, lineHeight: 1.4 }),
  /** Value text */
